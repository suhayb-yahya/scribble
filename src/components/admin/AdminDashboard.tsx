...
export default function AdminDashboard() {
  ...
  const saveJob = async (e: React.FormEvent) => {
    ...
    if (!titleEn && !titleAr) {
      setError("At least one title (EN or AR) is required");
      return;
    }
    setSaving(true);
    setError("");
    const payload = {
      title: { en: titleEn || titleAr, ar: titleAr || titleEn },
        ...
        <form onSubmit={saveJob} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-[#7B2553] uppercase tracking-wide">English</h3>
              <div>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-[#7B2553] uppercase tracking-wide">العربية</h3>
              <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Apply URL (optional, shared for both languages)
            </label>
            <input
              type="url"
              value={formApplyUrl}
        ...
        {jobsLoading ? (
          ...
          <ul className="space-y-4">
            {jobs.map((job) => (
              ...
              >
                <div>
                  <p className="font-semibold text-gray-900">{job.title.en || job.title.ar}</p>
                  {job.title.ar && job.title.en && (
                    <p className="text-sm text-gray-600">{job.title.ar}</p>
                  )}
